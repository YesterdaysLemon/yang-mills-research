...
Note 0035 tests the obvious scale-envelope repair and rules it out in its naive form. Weighting raw and gradient output occurrences by \(s_j\) and \(s_j^2\) cancels Eq. (190)'s printed powers, and an exact weighted cutoff gives a physical-width quotient constant \(1+c_{\rm Ad}^{\rm RG}\Theta/\rho\). But on the finest scale a one-bond complex direction has \(O(1)\) weighted norm and an \(O(1)\) plaquette change, while RG I permits only \(O(\xi^2)\). The full weighted analytic radius is therefore at most \(O(\xi^2)\), and generic Cauchy restores the same \(\xi^{-2}\) loss. An unweighted curl row does not help because a flat longitudinal step has zero curl and violates the unweighted gradient ceiling. This leaves structured cancellation, a range-sensitive derivative theorem, a justified physical-source restriction, or structural exclusion of the offending finest rows; Note 0036 tests that boundary further.

Note 0036 tests the strongest exact Ward-cancellation version of that route.
It combines each physical \(U\) column with its induced auxiliary-\(J\)
variation before absolute summation and quotients the product tube jointly
by inactive and simultaneous-gauge directions. Conditional on the completed
Ward identity, the resulting quotient-synthesis norm is the exact sharp
coefficient-independent constant; separately absolutizing the two summands
can destroy a real cancellation. Propagators-II vertical decay can transfer
the output powers to the input scale, but the same-layer same-cell label
retains the finest loss. A gauge-invariant transverse logical countermodel
...
Note 0036 replaces separate \(U\)- and \(J\)-absolute bounds by the joint
physical columns
\[
V_\alpha=(A_\alpha,L_{\bar U}A_\alpha).
\]
After quotienting the normalized product tube by coefficient-inactive and
simultaneous-gauge directions \(H_F\), the exact missing constant is
\[
\left\|Q_{H_F}K_F^w\right\|_{\ell^\infty\to E_\xi/H_F}.
\]
This preserves internal Ward cancellation, but arbitrary source phases
still align different column values. Spending vertical decay transfers
\(s_j^{-q}\) to \(L^qs_{j'}^{-q}\), while the diagonal
\(j'=j,y'=y\) retains the finest-scale power. A bounded local
